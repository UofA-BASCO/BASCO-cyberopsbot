...
function addRole (msg, args, username) {
  ...
  var mentionedRole = null
  if (!args[0]) return 'please add the role name.'

  if (args[0] && args[1] && args[2]) {
    mentionedRole = msg.guild.roles.cache.find(r => r.name === args[0] + ' ' + args[1] + ' ' + args[2])
    if (!mentionedRole) {
    ...
    mentionedRole = msg.guild.roles.cache.find(r => r.name === args[0] + ' ' + args[1])
    if (!mentionedRole) {
    ...
    mentionedRole = msg.guild.roles.cache.find(r => r.name === args[0])
    if (!mentionedRole) {
